...
def confirm_email(token):
    ...
    if request.method == 'GET':
        return '<form action="/" method="POST"><input name="password"><input type="submit"></form>'
    
    return '<h1>The token works!</h1>'
    '''reset_args = login_args.copy()
    reset_args.add_argument('token')
    #body = request.get_json()
    args = reset_args.parse_args()
    token = args.get('token')
    password = args.get('password')

    if not token or not password:
        return {'msg': 'The token is expired!'}
    ...
    with app.app_context():
        ...
        db.session.commit()
    msg = Message('Password reset successful', sender='zihuijiang6@gmail.com', recipients=[email])
    msg.body = 'Password reset was successful'
    mail.send(msg)

    return '<p>Password reset was successful</p>'
'''
